import { formatBRL, isValidAccountType, isValidAmount, isValidDescription, parseAmount, } from '../utils/validation.js';
...
function ensureValidAmount(amountInput) {
...
function ensureAccountOwnership(account, userId) {
    if (!account) {
...
function toAccountResponse(account, owner) {
    return {
        id: account.id,
...
function toBalanceResponse(account) {
    return {
...
export function createAccountService(repository) {
    return {
        ...
        async deposit(userId, accountId, input) {
            const account = ensureAccountOwnership(await repository.findAccountById(accountId), userId);
        ...
        async withdraw(userId, accountId, input) {
            const account = ensureAccountOwnership(await repository.findAccountById(accountId), userId);
            const amount = ensureValidAmount(input.amount);
            const description = normalizeOptionalDescription(input.description);
            const balanceBefore = Number(account.balance);
        ...
        async transfer(userId, accountId, input) {
            const fromAccount = ensureAccountOwnership(await repository.findAccountById(accountId), userId);
            const toAccount = await repository.findAccountById(input.toAccountId);
            if (!toAccount) {
        ...
        async getStatement(userId, accountId, input) {
            const account = ensureAccountOwnership(await repository.findAccountById(accountId), userId);
